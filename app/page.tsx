  ...
  if(rawTag) {
    try {
      ...
      const response = await fetch(url, {
        ...
        // Revalidate every 60 seconds so stats stay relatively fresh
        next: { revalidate: 60 } 
      });

      // Handle HTTP errors (like a 404 Not Found)
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error("Player not found. Double-check the tag!");
        } else if (response.status === 403) {
          throw new Error("Access denied. Check your API key and IP restrictions.");
        } else {
    ...
    playerData = await response.json();
      ...
      // 5. Catch network failures or the custom errors we threw above
      console.error("Fetch failed:", error);
      errorMessage = error instanceof Error ? errorMessage = error.message : "An unexpected error occurred.";
    }
  }
 // 6. Render the UI
  return (
    ...
    <main className="min-h-screen br-linear-to-br from-slate-900 via-indigo-950 to-purple-950 text-white p-8 font-sans selection:bg-pink-500">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Search Component Wrapper (Assuming your SearchForm handles dark mode!) */}
        ...
        {playerData && (
          <div className="space-y-8 animate-fade-in-up">
            ...
            {/* SECTION 1: Player Profile (Glassmorphism Card) */}
            ...
            {/* SECTION 3: Current Deck (Interactive Hover Cards) */}
                ...
                <div className="grid grid-cols-4 md:grid-cols-8 gap-4">
                  {playerData.currentDeck.map((card : Card) => 
                    ...
                    >
                      ...
                      />
                      <p className="text-[11px] text-center font-bold text-indigo-200 mt-3 uppercase tracking-wider">{card.name}</p>
                      <p className="text-[10px] text-pink-400 font-bold mt-1">Lvl {card.level}</p>
                    </div>
                  )}
                </div>
              </div>
